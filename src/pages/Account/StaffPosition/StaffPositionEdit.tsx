...
const StaffPositionEdit = () => {
  ...
  const selectStaffPositionProperties = createSelector(
    selectLayoutState,
    (state) => ({
      staffPosition: state.staffPosition,
      error: state.error,
    })
  );
  ...
  const { staffPosition, error } = useSelector(selectStaffPositionProperties);
  ...
  useEffect(() => {
    if (id) {
      dispatch(getStaffPositionById(Number(id)));
    }
  }, [dispatch, id]);
  ...
  const validation: any = useFormik({
    ...
    initialValues: {
      positionName: staffPosition?.positionName || "",
      code: staffPosition?.code || "",
      description: staffPosition?.description || "",
      level: staffPosition?.level || "Junior",
      department: staffPosition?.department || "",
      status: staffPosition?.status || "Published",
    },
    ...
    onSubmit: (values) => {
      if (id) {
        ...
        dispatch(updateStaffPosition(updatedData));
        setIsUpdated(true);
      }
    },
  });
  ...
  useEffect(() => {
    if (isUpdated) {
      showSuccess("Staff position updated successfully");
      setTimeout(() => {
        navigate("/account/staff-positions");
      }, 1000);
    }
  }, [isUpdated, navigate, showSuccess]);

  useEffect(() => {
    if (error) {
      showError("Failed to update staff position");
    }
  }, [error, showError]);
  ...
  document.title =
    "Edit Staff Position | Velzon - React Admin & Dashboard Template";

  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Edit Staff Position" pageTitle="Staff Positions" />
          <Row>
            <Col lg={12}>
              <Form
                onSubmit={(e) => {
              ...
              >
                <Card>
                  <CardHeader>
                    <h5 className="card-title mb-0">Edit Staff Position</h5>
                  </CardHeader>
                  <CardBody>
                    <Row>
                      <Col md={6}>
...
export default StaffPositionEdit;
